...
The `+android.18` Development candidate is stacked on the V1.2 source migration, `+android.8` accessibility-chrome hardening, `+android.9` large-text reflow work, `+android.10` RTL directionality hardening, `+android.11` string-resource localization readiness, `+android.12` debug pseudolocale testability, `+android.13` bidi-presentation hardening, `+android.14` explicit-port presentation correctness, `+android.15` navigation-authority validation, `+android.16` malformed-web external-handoff hardening, and `+android.17` bare-userinfo search-routing hardening. It additionally keeps known Browser-local/active-content schemes (`about:`, `blob:`, `content:`, `data:`, `file:`, and `javascript:`) inside Browser instead of making them eligible for Android external-app handoff merely because a page request has a user gesture.

The parent debug beta still generates synthetic Android pseudolocales `en-XA` and `ar-XB` for localization stress testing. Pseudolocales are test resources, not translations. Their presence in the APK does not establish rendered pseudolocale behavior, translation quality, or localization acceptance. Likewise, the address-presentation, navigation-authority, external-handoff, bare-input classification, and bounded local-scheme hardening do not establish complete URL/userinfo safety, email detection, phishing protection, URL-spoofing, DNS, origin, IDN/confusable, certificate, complete scheme/intent/deep-link safety, external-application trust, or bidirectional-editing acceptance.
...
A successful GoreeCloud Browser Android CI run proves only the checks performed by that workflow for the exact source revision: unit tests, Android lint, APK assembly, signature/package/version verification, debug pseudolocale resource presence, checksum/source-revision generation, and artifact creation. For `+android.18`, unit tests cover the inherited malformed explicit HTTP(S) authority fail-closed behavior including explicit web schemes without `//`, bounded authority-port validation, valid Unicode direct navigation, bare `localhost:port` and bracketed-IPv6-with-port HTTPS upgrade, invalid bare-port handling, malformed HTTP(S) external-handoff rejection, bare email-shaped input remaining GoreeCloud Search text, explicit HTTP(S) userinfo remaining outside the bare-host inference change, parseable-scheme requirements for external handoff, blocked `about:`/`blob:`/`content:`/`data:`/`file:`/`javascript:` handoff, gesture-gated `mailto:`/`tel:`/`intent:` eligibility, and Activity source wiring to the Browser-owned handoff policy. CI does not by itself establish production security, privacy, accessibility, rendered large-text behavior, localization, pseudolocale rendering, translation quality, email validation, complete URL/userinfo or phishing safety, DNS validity, certificate/origin trust, complete Unicode/IDN/confusable handling, complete scheme/intent/deep-link or external-application trust, rendered RTL directionality, bidirectional editing, real-device compatibility, recovery, or Stable qualification.