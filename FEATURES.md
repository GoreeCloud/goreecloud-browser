...
- Current stacked Development candidate identity `0.1.0-beta.1+android.18` / versionCode `10018`.
...
The string-resource extraction and pseudolocales are localization testability only. Pseudolocales are synthetic Android test resources, not translations. The bidi sanitizer and explicit-port preservation are presentation-only hardening, while navigation-authority, bare-userinfo routing, and external-handoff validation are local routing/syntax boundaries only. The local-scheme deny set narrows externalization but does not establish complete scheme safety. These changes do not establish email-address validation, complete URL/userinfo safety, phishing protection, DNS validity, certificate trust, origin safety, Wardveil trust, complete scheme safety, intent/deep-link safety, trust in external applications, translation completeness/quality, locale fallback/plural/grammar behavior, complete Unicode/IDN/confusable handling, bidirectional URL/address editing acceptance, rendered pseudolocale or RTL behavior, or representative locale/device acceptance.
...
- Complete URL/userinfo, email-classification/phishing, scheme/intent/deep-link review, and representative external-application handoff acceptance.
