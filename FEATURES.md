...
- Current stacked Development candidate identity `0.1.0-beta.1+android.19` / versionCode `10019`.
...
- Explicit malformed `http:` / `https:` input without `//` is also treated as a failed web-navigation attempt and goes to Search Home rather than query text.
...
- Unschemed input whose authority-shaped prefix contains literal `@` is not inferred as a bare host; email-shaped input such as `person@example.com` remains GoreeCloud Search text instead of being silently upgraded to an HTTPS userinfo URL.
- Explicit HTTP(S) userinfo URLs remain governed by the explicit-URL contract and are intentionally outside the bare-host inference correction.
...
- Browser-owned `WebViewClient` routing keeps malformed HTTP(S) page-link attempts inside Browser and does not hand them to another Android app through `ACTION_VIEW`.
- External Android handoff requires a user gesture plus a parseable eligible non-web scheme.
- `about:`, `blob:`, `content:`, `data:`, `file:`, and `javascript:` remain Browser-owned even when a user gesture is present; `mailto:`, `tel:`, and inherited `intent:` targets retain gesture-gated eligibility.
- Browser-intent handling for structurally allowed HTTP/HTTPS links.
- Browser-owned Copy page address and Share page actions pass the current page through `AddressDisclosurePolicy`, removing explicit HTTP(S) userinfo before outward clipboard/share disclosure while preserving the remaining scheme, host, explicit port, path, query, fragment, and raw escaping.
- Focused omnibox editing and navigation authority retain the untouched exact URL, including explicit HTTP(S) userinfo; the disclosure policy does not rewrite WebView navigation or history state.
...
- The same source contract requires `BrowserActivity` to consume the Browser-owned external-handoff policy and rejects the prior unconditional `request.hasGesture()` externalization form.
- A separate outward-disclosure source contract requires both Copy page address and Share page to use `AddressDisclosurePolicy.forExternalUse(currentUrl)` rather than exporting raw `currentUrl`.
- Debug beta pseudolocale generation is enabled for Android `en-XA` and `ar-XB`; the release build type is not opted into this debug testability contract.
- The localization source contract also fails closed unless the debug-only pseudolocale boundary remains configured.
- Android CI verifies pseudolocalized `en-XA` and `ar-XB` application-label resources plus both locale identifiers in the built debug APK's `aapt` badging before artifact upload.
...
- Focused omnibox editing continues to expose the untouched authoritative `currentUrl`; presentation sanitization/condensing and outward disclosure hardening do not rewrite navigation, search resolution, WebView requests, persisted state, or network behavior.
- Unit tests for Browser-owned navigation resolution including malformed explicit authority fail-closed behavior, malformed web-scheme handling without `//`, Unicode direct navigation, localhost/IPv6 explicit-port bare-host upgrades, invalid-port handling, bare email-shaped input remaining Search text, explicit HTTP(S) userinfo non-regression, blocked local/active-content external-handoff schemes, parseable-scheme requirements, and gesture-gated eligible external schemes; outward disclosure including HTTP(S) credential/userinfo stripping, raw-escaping preservation, explicit-port/path/query/fragment preservation, ordinary HTTP(S) non-regression, and unchanged non-web/malformed values; address presentation including explicit-port preservation and bidi-control sanitization; GLAZE UI mapping; accessibility-aware top-chrome visibility policy; large-text content-height source contract; bounded RTL directionality source contract; Android string-resource localization foundation; debug pseudolocale configuration; Activity handoff/disclosure-policy wiring; and focused-vs-unfocused URL source separation.
...
The string-resource extraction and pseudolocales are localization testability only. Pseudolocales are synthetic Android test resources, not translations. The bidi sanitizer and explicit-port preservation are presentation-only hardening, while navigation-authority, bare-userinfo routing, external-handoff validation, and outward address disclosure are bounded local policy boundaries. The local-scheme deny set narrows externalization but does not establish complete scheme safety. The outward-disclosure policy reduces accidental export of explicit HTTP(S) userinfo but does not establish complete credential safety, clipboard privacy, Android share-target trust, complete URL/userinfo safety, or phishing protection. These changes do not establish email-address validation, DNS validity, certificate trust, origin safety, Wardveil trust, complete scheme safety, intent/deep-link safety, trust in external applications, translation completeness/quality, locale fallback/plural/grammar behavior, complete Unicode/IDN/confusable handling, bidirectional URL/address editing acceptance, rendered pseudolocale or RTL behavior, or representative locale/device acceptance.
...
- Browser-owned navigation resolution requires structurally valid HTTP(S) authority syntax and bounded optional ports before a URL is reported allowed; malformed HTTP(S) page-link attempts also remain Browser-owned instead of qualifying for external-app handoff; unschemed literal-`@` input is not automatically inferred as a bare host; and known local/active-content schemes remain ineligible for Android external handoff. These are navigation-policy source contracts, not Glaze security, email, URL/userinfo, or complete scheme-safety authority.
- Browser-owned outward page-address disclosure removes explicit HTTP(S) userinfo from Copy/Share output while leaving navigation and focused editing untouched; this is a privacy/disclosure source contract, not Glaze credential, clipboard, or share-target authority.
...
- Complete URL/userinfo/credential-disclosure, clipboard/share-target, email-classification/phishing, scheme/intent/deep-link review, and representative external-application handoff acceptance.
