...
- version: `0.1.0-beta.1+android.20`
- versionCode: `10020`
- minimum Android API: 26
- target Android API: 35

The Android stack currently consists of the V1.2 Stable mapping parent, the `+android.8` accessibility-chrome persistence slice, the `+android.9` large-text content-height hardening slice, the `+android.10` RTL-directionality hardening slice, the `+android.11` Android string-resource localization-readiness slice, the `+android.12` debug pseudolocale testability slice, the `+android.13` unfocused address bidi-presentation hardening slice, the `+android.14` explicit-port presentation-correctness slice, the `+android.15` navigation-authority validation slice, the `+android.16` malformed-web external-handoff fail-closed slice, the `+android.17` bare-userinfo search-routing slice, the `+android.18` local/active-content external-handoff deny slice, the `+android.19` outward page-address disclosure hardening slice, and this `+android.20` URL-fragment presentation-correctness slice. Android System WebView/Chromium is used only as the rendering engine dependency. GoreeCloud owns the mobile chrome, URL/search routing, privacy defaults, permission behavior, design-system mapping, and product capability gates. Downloads remain fail-closed until the Android path satisfies the authoritative Wardveil release contract.

The `+android.9` source replaces exact fixed heights on text-bearing top/menu chrome with Android-native content-height measurement while retaining minimum interaction floors. The `+android.10` source fixes directional history glyph behavior by enabling Android-native auto-mirroring for Back and Forward and guards logical start/end horizontal-margin usage. The `+android.11` source moves Browser-owned visible and accessibility copy out of `BrowserActivity.kt` into Android string resources, while HTTPS/HTTP/WEB protocol/status tokens remain explicitly non-translatable. The `+android.12` debug build enables Android pseudolocale generation so the debug APK includes `en-XA` and `ar-XB` resource variants for later expansion/accent and RTL stress testing on supported test devices. The release build type is not opted into this debug pseudolocale contract. The `+android.13` source removes Unicode bidirectional-formatting controls from the condensed **unfocused** address presentation while preserving ordinary RTL letters and leaving the authoritative current URL unchanged; focusing the omnibox continues to expose the original full URL for editing. The `+android.14` source additionally preserves every explicitly parsed URL port in that condensed presentation—including an explicitly written default port—so the Browser does not visually collapse `example.com:8443` or `example.com:443` to a host-only presentation. The `+android.15` source validates HTTP(S) authority shape before Browser reports a URL allowed, validates optional ports in the `0..65535` range, correctly recognizes bare `localhost:port` and bracketed IPv6-with-port input for HTTPS upgrade, preserves valid Unicode HTTP(S) host input without performing IDN normalization, and fails malformed **explicit** HTTP(S) navigation to GoreeCloud Search Home instead of forwarding malformed authority text to WebView or converting that explicit navigation attempt into a search query. The `+android.16` source extends that fail-closed boundary to page-link interception: HTTP(S) attempts remain Browser-owned even when malformed, while Android external-app handoff is eligible only for a user-gestured non-web scheme. It also treats malformed explicit `http:` / `https:` input without `//` as an explicit web-navigation attempt that fails to Search Home rather than query text. The `+android.17` source prevents unschemed input containing literal `@` from being inferred as a bare host, so email-shaped text such as `person@example.com` remains GoreeCloud Search input instead of being silently upgraded to `https://person@...`; explicit HTTP(S) userinfo URLs remain outside this narrow bare-host inference correction. The `+android.18` source further requires a parseable scheme before external handoff can be eligible and keeps `about:`, `blob:`, `content:`, `data:`, `file:`, and `javascript:` Browser-owned even when a user gesture is present, while retaining gesture-gated eligibility for external schemes such as `mailto:`, `tel:`, and the inherited `intent:` path. The `+android.19` source adds a Browser-owned outward disclosure policy: explicit HTTP(S) userinfo remains part of the exact authoritative URL for navigation and focused editing, but Copy page address and Share page remove only that userinfo before the address leaves Browser while preserving the remaining scheme, host, explicit port, path, query, fragment, and raw escaping. The `+android.20` source preserves `URI.rawFragment` in the condensed unfocused address so distinct same-document targets such as `example.com/page#section-a` and `example.com/page#section-b` no longer collapse to the same visible address; percent-escaped fragment text remains raw and the combined display continues through the existing bidi-control sanitizer.

JVM source contracts guard the reviewed localization resource surface, the debug-only pseudolocale configuration, the separation between sanitized unfocused presentation and the untouched focused/current URL, explicit-port and fragment presentation behavior, navigation-authority resolution cases, malformed-web external-handoff policy, bare-userinfo search-routing behavior, the bounded local/active-content scheme handoff deny policy, and outward HTTP(S) userinfo disclosure behavior. Source-wiring assertions require `BrowserActivity` to consume the Browser-owned handoff policy and require both Copy page address and Share page to use the outward disclosure policy rather than exporting raw `currentUrl`. Android CI verifies that the built debug APK reports the expected package/version identity and both pseudolocale resource variants through `aapt` before artifact upload.

These changes are source/runtime presentation, navigation-policy, handoff-routing, bare-input classification, local scheme classification, outward-address disclosure, and localization-testability hardening only. Pseudolocales are synthetic test resources, not translations. Authority-shape validation is not DNS validation, IDN/confusable safety, certificate verification, site trust, or a Wardveil verdict. The external-handoff rules do not establish complete scheme safety, intent/deep-link safety, or trust in external applications. The bare-userinfo rule is not email validation, complete URL/userinfo safety, credential handling, or phishing protection. The outward-disclosure rule does not establish complete credential safety, clipboard privacy, Android share-target trust, or complete URL/userinfo safety. Fragment preservation is display correctness only and does not establish fragment trust, origin safety, anti-spoofing, bidirectional editing acceptance, or rendered-device acceptance. Representative rendered 200% text, actual translations, translation quality/completeness, locale fallback/grammar, complete origin/Unicode/IDN/confusable handling, bidirectional URL/address editing behavior, rendered RTL/pseudolocale behavior, assistive-technology behavior, and combined locale/device acceptance remain pending.
...
The current Android branch stack advances Browser from its historical Glaze UI 2.2 and V1.1 mappings to V1.2 Stable authority, preserves the working mobile shell, protects primary top chrome while Android accessibility services are active, hardens text-bearing chrome against fixed-height clipping under system font scaling, adds bounded RTL directionality hardening for directional history controls, makes Browser-owned natural-language chrome copy resource-backed for future localization, equips the debug APK with Android pseudolocales for later localization stress testing, hardens condensed address presentation against embedded bidi-formatting controls without rewriting navigation authority, preserves explicitly specified URL ports and URL fragments in the condensed presentation, hardens Browser-owned web-navigation authority-shape validation, keeps malformed HTTP(S) page-link attempts inside the Browser fail-closed boundary instead of externalizing them to Android applications, keeps unschemed email-shaped input on the normal Search path instead of converting it into URL userinfo navigation, keeps known Browser-local/active-content schemes out of Android external-app handoff, and prevents explicit HTTP(S) userinfo from being silently exported by Browser-owned Copy/Share actions while leaving navigation authority untouched.
...
- condensed unfocused address presentation strips Unicode bidi-formatting controls while ordinary RTL letters are preserved, explicitly parsed ports and raw fragments remain visible, and the authoritative full URL remains available unchanged on focus;
...
Presentation never manufactures Wardveil security truth, Privacy Shield privacy truth, Everkeep continuity truth, Identity authorization, Mesh coordination, Search authority, Sync authority, or Browser workflow success. Address presentation, navigation-authority, bare-host inference, external-handoff routing, and outward-address disclosure validation likewise do not manufacture email semantics, complete URL/userinfo safety, credential safety, DNS, certificate, IDN/confusable, origin, fragment trust, complete scheme/intent/deep-link safety, clipboard privacy, or external-application trust.

This is repository-local source mapping and hardening only. Browser still requires exact-revision rendered/native visual review, TalkBack, Switch Access, Voice Access, rendered 200% text, actual translated/localized copy and translation-quality review, locale fallback/grammar behavior, complete origin/Unicode/IDN/confusable review, complete URL/userinfo and credential-disclosure review, complete scheme/intent/deep-link and external-application handoff review, bidirectional URL/address behavior including fragments, rendered RTL and pseudolocale review, Reduced Motion, Reduced Transparency/effects-free behavior, contrast/high-contrast handling, Touch Assistance mapping where supported, form-factor behavior, performance, representative physical-device evidence, and product-specific production approval.
...
- **Browser Engine Layer** — GoreeCloud-owned interface between Browser application logic and rendering/runtime foundations.
...
The current Android beta provides Browser-owned Back, Forward, Home, Reload/Stop, a unified address/search field, page progress, structurally validated direct HTTP/HTTPS navigation, bare-host HTTPS upgrade including `localhost:port` and bracketed IPv6-with-port input, GoreeCloud Search routing for non-URL and email-shaped unschemed input, fail-closed handling of malformed explicit HTTP(S) authority input, Browser-owned blocking of malformed HTTP(S) page-link attempts, user-gesture-gated external handoff for eligible non-web schemes with known local/active-content schemes blocked, browser-intent handling for structurally allowed web links, Browser-owned menu actions, clipboard/share page-address disclosure that removes explicit HTTP(S) userinfo before the address leaves Browser, accessibility-aware scroll chrome, content-height text-bearing top/menu chrome, bounded native RTL directionality for Back/Forward history controls, a resource-backed Browser-owned UI-copy foundation for future localization, debug pseudolocale resources for later test-device localization stress testing, presentation-only removal of Unicode bidi-formatting controls from the condensed unfocused address, and preservation of explicitly parsed URL ports and raw URL fragments in that condensed presentation while retaining the original full URL for focused editing/navigation authority.

Security/privacy defaults remain deliberately conservative:

- malformed explicit HTTP(S) input fails to GoreeCloud Search Home instead of being forwarded to WebView or sent as a search query;
...
- unschemed input containing literal `@` is not inferred as a bare host or silently upgraded into an HTTPS userinfo URL;
...
- Browser-owned Copy page address and Share page remove explicit HTTP(S) userinfo before disclosure while navigation and focused editing preserve the exact authoritative URL;
...
Private Browsing, full tab-management UI, accepted permission surfaces, production Wardveil downloads, full first-party runtime integrations, actual locale translations, representative pseudolocale/large-text/assistive-technology/localization/RTL acceptance, complete DNS/origin/Unicode/IDN/confusable and bidirectional editing acceptance, complete URL/userinfo/credential-disclosure/email-classification/phishing acceptance, complete clipboard privacy and Android share-target trust acceptance, complete fragment/origin presentation acceptance, complete scheme/intent/deep-link and external-application trust acceptance, managed updates/rollback, production signing, store distribution, and Stable qualification remain incomplete.
