...
JVM source contracts guard the reviewed localization resource surface, the debug-only pseudolocale configuration, the separation between sanitized unfocused presentation and the untouched focused/current URL, explicit-port presentation behavior, navigation-authority resolution cases, malformed-web external-handoff policy, and bare-userinfo search-routing behavior. A source-wiring assertion also requires `BrowserActivity` to consume the Browser-owned handoff policy rather than reverting to unconditional gesture-based externalization. Android CI verifies that the built debug APK reports the expected package/version identity and both pseudolocale resource variants through `aapt` before artifact upload.

These changes are source/runtime presentation, navigation-policy, handoff-routing, bare-input classification, and localization-testability hardening only. Pseudolocales are synthetic test resources, not translations. Authority-shape validation is not DNS validation, IDN/confusable safety, certificate verification, site trust, or a Wardveil verdict. The external-handoff rule does not establish complete scheme safety, intent/deep-link safety, or trust in external applications. The bare-userinfo rule is not email validation, complete URL/userinfo safety, credential handling, or phishing protection. Representative rendered 200% text, actual translations, translation quality/completeness, locale fallback/grammar, complete origin/Unicode/IDN/confusable handling, bidirectional URL/address editing behavior, rendered RTL/pseudolocale behavior, assistive-technology behavior, and combined locale/device acceptance remain pending.
