...
The Android shell now has a **source-level Glaze UI 2.0.0 native mapping candidate** using native Android controls, current material semantics, a 48dp general target floor, light/dark adaptation, focus/pressed states, and effects-free fallback behavior. That mapping is not yet native-device Glaze acceptance; representative visual, accessibility, form-factor, and real-device evidence remains required.
