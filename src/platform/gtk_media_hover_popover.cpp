...
const char* action_label(MediaAction action) {
  switch (action) {
    ...
    case MediaAction::search_similar: return "Find Visually Similar";
    case MediaAction::search_region: return "Search Selected Area";
    case MediaAction::search_frame: return "Search Current Frame";
    case MediaAction::save: return "Save";
    case MediaAction::save_region: return "Save Selected Area";
    case MediaAction::save_frame: return "Save Current Frame";
    ...
    case MediaAction::copy_region: return "Copy Selected Area";
    case MediaAction::copy_frame: return "Copy Current Frame";
    ...
    case MediaAction::ocr: return "Extract Text (OCR)";
    case MediaAction::translate_text: return "Translate Text";
    case MediaAction::read_aloud: return "Read Text Aloud";
    case MediaAction::ask_goreecloud_ai:
    case MediaAction::ask_ai: return "Ask GoreeCloud AI";
    case MediaAction::describe_media: return "Generate Description";
    ...
    case MediaAction::save_to_notes: return "Add to GoreeCloud Notes";
    case MediaAction::save_to_memos: return "Add to GoreeCloud Memos";
    case MediaAction::create_task: return "Create GoreeCloud Task";
    case MediaAction::save_to_drive: return "Save to GoreeCloud Drive";
    case MediaAction::save_to_photos: return "Save to GoreeCloud Photos";
    case MediaAction::save_to_video: return "Save to GoreeCloud Video";
...
void on_action_clicked(GtkButton* button, gpointer data) {
  ...
  if (it->second == MediaAction::more) {
    state->expanded = !state->expanded;
    if (state->expanded) gtk_widget_show_all(state->more_box);
    else gtk_widget_hide(state->more_box);
    return;
  }
...
GtkWidget* make_action_button(PopoverState& state,
  ...
  if (auto* accessible = gtk_widget_get_accessible(button)) atk_object_set_name(accessible, label);
...
void install_css(GtkWidget* parent) {
  ...
  static constexpr const char* kCss = R"CSS(
    .gc-media-hover-popover contents { border-radius: 16px; padding: 8px; background-color: @theme_base_color; border: 1px solid alpha(@theme_fg_color, 0.16); box-shadow: 0 8px 28px alpha(#000000, 0.24); }
    .gc-media-hover-action { min-width: 44px; min-height: 44px; border-radius: 14px; padding: 6px 12px; }
    .gc-media-hover-chip { border-radius: 12px; padding: 4px 8px; background-color: alpha(@theme_fg_color, 0.08); }
    .gc-media-hover-summary { padding: 2px 4px 6px 4px; }
  )CSS";
...
PopoverState& ensure_state(GtkWidget* parent) {
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(state->popover), "gc-media-hover-popover");
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(state->summary), "gc-media-hover-summary");
...
void show_gtk_media_hover_popover(void* parent_widget,
  ...
  for (const auto& quick : model.quick_actions) {
    auto* button = make_action_button(state, quick.action, quick.label.c_str(), quick.enabled);
  ...
  GdkRectangle rect{viewport_x, viewport_y, 1, 1};
  gtk_popover_set_pointing_to(GTK_POPOVER(state.popover), &rect);
  gtk_widget_show_all(state.popover);
  if (!state.expanded) gtk_widget_hide(state.more_box);
}
