...
- Browser beta identity for this mapping/hardening stack: `0.1.0-beta.1+android.18` / versionCode `10018`
...
GLAZE UI V1.2 / `1.2.0` became the current Stable consumer target on September 6, 2026. Browser's earlier Glaze UI 2.2 and V1.1 mappings remain historical source/rollback evidence only. The Android Development stack now layers the V1.2 source migration (`+android.7`), accessibility-aware top-chrome persistence (`+android.8`), large-text content-height hardening (`+android.9`), bounded RTL history-control directionality hardening (`+android.10`), Browser-owned Android string-resource localization readiness (`+android.11`), debug pseudolocale testability (`+android.12`), bounded unfocused address bidi-presentation hardening (`+android.13`), explicit-port presentation correctness (`+android.14`), Browser-owned navigation-authority validation (`+android.15`), malformed-web external-handoff fail-closed hardening (`+android.16`), bare-userinfo search-routing hardening (`+android.17`), and bounded local/active-content external-handoff deny hardening (`+android.18`). Central design-system Stable status and these source changes do not auto-promote Browser or satisfy Browser-specific downstream acceptance.
...
- Navigation-authority, bare-userinfo routing, and external-handoff validation are **not** Glaze authorities. They do not perform email validation, complete URL/userinfo validation, phishing analysis, DNS resolution, certificate validation, IDN normalization/confusable analysis, origin-trust classification, Wardveil classification, complete scheme/intent/deep-link validation, external-application trust classification, or network requests, and Glaze presentation must never convert them into such claims.
- The localization-resource, pseudolocale, bidi-presentation, explicit-port, navigation-authority, malformed-web handoff, bare-userinfo, and local-scheme handoff hardening create testing/source boundaries only. They do not establish actual translations, translation completeness/quality, locale fallback/plural/grammar behavior, email validity, complete URL/userinfo or phishing safety, DNS validity, certificate/origin trust, complete Unicode/IDN/confusable handling, complete scheme/intent/deep-link or external-application trust, bidirectional URL/address editing, rendered pseudolocale behavior, rendered RTL layout, or representative locale/device acceptance.
...
Appearance, material, motion, color, layout, accessibility-source, directionality-source, localization-resource, pseudolocale-testability, bidi-address-presentation, and explicit-port-presentation contracts are presentation/interaction/test contracts only. Browser navigation-authority, bare-userinfo search-routing, malformed-web external-handoff validation, and local-scheme external-handoff validation are separate routing-policy source contracts. Neither class of contract can manufacture Wardveil security state, Privacy Shield privacy state, Everkeep continuity state, Mesh coordination state, Identity authorization, Search authority, Sync authority, email truth, URL/userinfo safety, DNS validity, certificate trust, origin trust, complete scheme/intent safety, or external-application trust.
