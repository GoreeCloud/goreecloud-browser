...
- Browser beta identity for this mapping/hardening stack: `0.1.0-beta.1+android.17` / versionCode `10017`
...
GLAZE UI V1.2 / `1.2.0` became the current Stable consumer target on September 6, 2026. Browser's earlier Glaze UI 2.2 and V1.1 mappings remain historical source/rollback evidence only. The Android Development stack now layers the V1.2 source migration (`+android.7`), accessibility-aware top-chrome persistence (`+android.8`), large-text content-height hardening (`+android.9`), bounded RTL history-control directionality hardening (`+android.10`), Browser-owned Android string-resource localization readiness (`+android.11`), debug pseudolocale testability (`+android.12`), bounded unfocused address bidi-presentation hardening (`+android.13`), explicit-port presentation correctness (`+android.14`), Browser-owned navigation-authority validation (`+android.15`), malformed-web external-handoff fail-closed hardening (`+android.16`), and bare-userinfo search-routing hardening (`+android.17`). Central design-system Stable status and these source changes do not auto-promote Browser or satisfy Browser-specific downstream acceptance.
...
Appearance, material, motion, color, layout, accessibility-source, directionality-source, localization-resource, pseudolocale-testability, bidi-address-presentation, and explicit-port-presentation contracts are presentation/interaction/test contracts only. Browser navigation-authority, bare-userinfo search-routing, and malformed-web external-handoff validation are separate routing-policy source contracts. Neither class of contract can manufacture Wardveil security state, Privacy Shield privacy state, Everkeep continuity state, Mesh coordination state, Identity authorization, Search authority, Sync authority, email truth, URL/userinfo safety, DNS validity, certificate trust, origin trust, scheme/intent safety, or external-application trust.
