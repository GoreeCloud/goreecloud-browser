...
- Browser beta identity for this mapping/hardening stack: `0.1.0-beta.1+android.15` / versionCode `10015`
...
- Browser-owned navigation now applies a separate local HTTP(S) authority-syntax boundary before direct navigation is allowed. Malformed explicit HTTP(S) input fails to GoreeCloud Search Home; optional authority ports must be decimal and within `0..65535`; valid `localhost:port`, bracketed IPv6-with-port, and Unicode HTTP(S) host inputs remain supported within this bounded source contract.
- Navigation-authority validation is **not** a Glaze authority. It does not perform DNS resolution, certificate validation, IDN normalization/confusable analysis, origin-trust classification, Wardveil classification, or network requests, and Glaze presentation must never convert it into such a claim.
- The localization-resource, pseudolocale, bidi-presentation, explicit-port, and navigation-authority hardening create testing/source boundaries only. They do not establish actual translations, translation completeness/quality, locale fallback/plural/grammar behavior, DNS validity, certificate/origin trust, complete Unicode/IDN/confusable handling, bidirectional URL/address editing, rendered pseudolocale behavior, rendered RTL layout, or representative locale/device acceptance.
...
Appearance, material, motion, color, layout, accessibility-source, directionality-source, localization-resource, pseudolocale-testability, bidi-address-presentation, and explicit-port-presentation contracts are presentation/interaction/test contracts only. Browser navigation-authority validation is a separate routing-policy source contract. Neither class of contract can manufacture Wardveil security state, Privacy Shield privacy state, Everkeep continuity state, Mesh coordination state, Identity authorization, Search authority, Sync authority, DNS validity, certificate trust, or origin trust.
