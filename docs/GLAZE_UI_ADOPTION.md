...
- Browser beta identity for this mapping/hardening stack: `0.1.0-beta.1+android.19` / versionCode `10019`
...
GLAZE UI V1.2 / `1.2.0` became the current Stable consumer target on September 6, 2026. Browser's earlier Glaze UI 2.2 and V1.1 mappings remain historical source/rollback evidence only. The Android Development stack now layers the V1.2 source migration (`+android.7`), accessibility-aware top-chrome persistence (`+android.8`), large-text content-height hardening (`+android.9`), bounded RTL history-control directionality hardening (`+android.10`), Browser-owned Android string-resource localization readiness (`+android.11`), debug pseudolocale testability (`+android.12`), bounded unfocused address bidi-presentation hardening (`+android.13`), explicit-port presentation correctness (`+android.14`), Browser-owned navigation-authority validation (`+android.15`), malformed-web external-handoff fail-closed hardening (`+android.16`), bare-userinfo search-routing hardening (`+android.17`), bounded local/active-content external-handoff deny hardening (`+android.18`), and outward page-address userinfo disclosure hardening (`+android.19`). Central design-system Stable status and these source changes do not auto-promote Browser or satisfy Browser-specific downstream acceptance.
...
- The address-presentation hardening is explicitly presentation-only and does not rewrite navigation, search resolution, WebView requests, persistence, permissions, telemetry, or network traffic.
...
- Unschemed input whose authority-shaped prefix contains literal `@` is not inferred as a bare host, so email-shaped text remains on the ordinary GoreeCloud Search path rather than being silently transformed into HTTPS userinfo navigation. Explicit HTTP(S) userinfo remains outside this bare-host inference correction.
- Malformed HTTP(S) page-link attempts remain Browser-owned and are not eligible for Android external-app handoff merely because a user gesture occurred.
- External handoff additionally requires a parseable eligible non-web scheme; `about:`, `blob:`, `content:`, `data:`, `file:`, and `javascript:` remain Browser-owned even when a user gesture is present, while `mailto:`, `tel:`, and inherited `intent:` targets retain gesture-gated eligibility.
- Browser-owned Copy page address and Share page apply a separate outward-disclosure policy. Structurally parseable HTTP(S) userinfo is removed only from the outward clipboard/share value; navigation state and focused omnibox editing retain the exact authoritative URL, and the remaining scheme, host, explicit port, path, query, fragment, and raw escaping are preserved.
- Navigation-authority, bare-userinfo routing, external-handoff validation, and outward-address disclosure are **not** Glaze authorities. They do not perform email validation, complete URL/userinfo or credential validation, phishing analysis, clipboard privacy enforcement, Android share-target trust classification, DNS resolution, certificate validation, IDN normalization/confusable analysis, origin-trust classification, Wardveil classification, complete scheme/intent/deep-link validation, external-application trust classification, or network requests, and Glaze presentation must never convert them into such claims.
- The localization-resource, pseudolocale, bidi-presentation, explicit-port, navigation-authority, malformed-web handoff, bare-userinfo, local-scheme handoff, and outward-disclosure hardening create testing/source boundaries only. They do not establish actual translations, translation completeness/quality, locale fallback/plural/grammar behavior, email validity, complete URL/userinfo/credential or phishing safety, clipboard privacy, Android share-target trust, DNS validity, certificate/origin trust, complete Unicode/IDN/confusable handling, complete scheme/intent/deep-link or external-application trust, bidirectional URL/address editing, rendered pseudolocale behavior, rendered RTL layout, or representative locale/device acceptance.
...
Appearance, material, motion, color, layout, accessibility-source, directionality-source, localization-resource, pseudolocale-testability, bidi-address-presentation, and explicit-port-presentation contracts are presentation/interaction/test contracts only. Browser navigation-authority, bare-userinfo search-routing, malformed-web external-handoff validation, local-scheme external-handoff validation, and outward-address disclosure are separate routing/privacy-policy source contracts. Neither class of contract can manufacture Wardveil security state, Privacy Shield privacy state, Everkeep continuity state, Mesh coordination state, Identity authorization, Search authority, Sync authority, email truth, complete URL/userinfo/credential safety, clipboard privacy, Android share-target trust, DNS validity, certificate trust, origin trust, complete scheme/intent safety, or external-application trust.
